...
& $env:BLENDER_EXE --background --python $forge -- --manifest $manifest --faction black --piece all --source-root $env:TUMBO_CHESS_SOURCE_ROOT --output-root $out --private-review-root $private --repository-root $repo --face-image $env:TUMBO_FACE_IMAGE
if ($LASTEXITCODE -ne 0) { throw "Black army forge failed: $LASTEXITCODE" }
Write-Host "Black army private review complete: $private"

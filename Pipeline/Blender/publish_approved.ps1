param([Parameter(Mandatory=$true)][ValidateSet('black','white','all')]$Faction)
...
& $env:BLENDER_EXE --background --python $forge -- --manifest $manifest --faction $Faction --piece all --source-root $env:TUMBO_CHESS_SOURCE_ROOT --output-root $out --private-review-root $private --face-image $env:TUMBO_FACE_IMAGE --publish-approved

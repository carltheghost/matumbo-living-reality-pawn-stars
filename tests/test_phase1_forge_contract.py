...
class Phase1ForgeContractTests(unittest.TestCase):
    ...
    def test_private_paths_and_exact_reviewed_publication_are_enforced(self):
        ...
        self.assertIn("$verified.Count -ne 12", promote)
        self.assertIn(".phase1-approved-staging", promote)
        self.assertIn("Publication transaction incomplete", promote)
        self.assertIn("$installed", promote)
        self.assertIn("Post-promotion hash mismatch", promote)
